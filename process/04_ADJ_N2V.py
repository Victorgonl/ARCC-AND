...
from src.utils import (
    generate_adj_matrix_by_rulesim,
    parse_configion,
    parseJson,
    saveJson,
)
...
train_rel_emb_rule_path = "{}/{}/{}".format(
    BasePath, config["processed_path"], config["train_rel_emb_rule"]
)
valid_rel_emb_rule_path = "{}/{}/{}".format(
    BasePath, config["processed_path"], config["valid_rel_emb_rule"]
)
test_rel_emb_rule_path = "{}/{}/{}".format(
    BasePath, config["processed_path"], config["test_rel_emb_rule"]
)
...
def generate_adj_and_n2v():
    ...
    for train_name in train_name_list:
        paper_ids = train_data_df[train_data_df["name"] == train_name][
            "paperid"
        ].values.tolist()
        adj_matrix = generate_adj_matrix_by_rulesim(
            paper_ids, paper_infos, idfMap, threshold=config["idf_threshold"]
        )
    ...
    for eval_name in valid_name_list:
        paper_ids = valid_data_df[valid_data_df["name"] == eval_name][
            "paperid"
        ].values.tolist()
        adj_matrix = generate_adj_matrix_by_rulesim(
            paper_ids, paper_infos, idfMap, threshold=config["idf_threshold"]
    ...
    for test_name in test_name_list:
        paper_ids = test_data_df[test_data_df["name"] == test_name][
            "paperid"
        ].values.tolist()
        adj_matrix = generate_adj_matrix_by_rulesim(
            paper_ids, paper_infos, idfMap, threshold=config["idf_threshold"]
